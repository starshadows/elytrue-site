...
import {
  ...
  DAILY_COMMENT_COUNT_VERSION,
  LATEST_COMMENT_COUNT,
  LATEST_COMMENT_REVISION_VERSION,
  isDailyCommentCount,
  isCommentCard,
  isLatestCommentRevision,
  isLatestCommentView,
  publicViewId,
  toCommentCard,
  userViewId,
...
export async function rebuildCommentViews(data, options = {}) {
  ...
  for (const comment of comments) {
    ...
    if (comment.uid ? !cardsEqual(userView, expected) : userView !== null) {
  ...
  const previous = await getJSON(data, blobKeys.commentsPreviousLatestView)
  const latestRevision = await getJSON(data, blobKeys.commentsLatestRevision)
  const revisionBaseline = Math.max(
    isLatestCommentRevision(latestRevision)
      ? Number(latestRevision.revision)
      : 0,
    Number.isSafeInteger(Number(latest?.snapshotRevision))
      ? Number(latest.snapshotRevision)
      : 0,
    Number.isSafeInteger(Number(previous?.snapshotRevision))
      ? Number(previous.snapshotRevision)
      : 0,
  )
  const repairedRevision = revisionBaseline + 1
  ...
  const latestExpected = {
    ...
    snapshotRevision: repairedRevision,
  ...
  const dailyCount = await getJSON(data, blobKeys.commentsDailyCount(date))
  if (
  ...
  if (!isLatestCommentView(previous)) {
    issues.push({ type: 'previous-latest-view' })
  }
  if (latestRevision !== null && !isLatestCommentRevision(latestRevision)) {
    issues.push({ type: 'latest-view-revision' })
  }
  if (
    !isDailyCommentCount(dailyCount, date) ||
    dailyCount.count !== dateBlobs.length
  ) {
    issues.push({ type: 'daily-comment-count', date })
  }
  if (fix) {
    await data.setJSON(blobKeys.commentsLatestRevision, {
      version: LATEST_COMMENT_REVISION_VERSION,
      revision: repairedRevision,
      updatedAt: Date.now(),
    })
    await data.setJSON(blobKeys.commentsPreviousLatestView, latestExpected)
    await data.setJSON(blobKeys.commentsDailyCount(date), {
      version: DAILY_COMMENT_COUNT_VERSION,
      date,
      count: dateBlobs.length,
      updatedAt: Date.now(),
    })
