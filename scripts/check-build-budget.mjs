...
evaluate(
  'total JS',
  javascript.reduce((total, record) => total + record.size, 0),
  budgets.javascriptTotal,
  failures,
  warnings,
)
console.log(
  `     gzip ${formatBytes(javascript.reduce((total, record) => total + (record.gzipSize || 0), 0))}`,
)
evaluate(
  'total CSS',
...
const firstBackground = largest(
  images.filter((record) =>
    /\/bg\/(?:landscape|portrait)\d+\.webp$/u.test(record.path),
  ),
)
