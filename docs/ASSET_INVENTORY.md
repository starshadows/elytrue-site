...
当前 `public/` 合计 64 个文件、84.72 MiB。最大单文件为 `originals/portrait3.png`（10.56 MiB），低于 EdgeOne Makers 25 MiB 限制。

## 保留但与主应用隔离的历史入口

| 路径         | 可达性                | 素材状态                                    | 处理结论                                                                                                                     |
| ------------ | --------------------- | ------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| `/yumeniwa/` | 可由公开 URL 直接访问 | 4 个文件、约 0.01 MiB；页面依赖历史外部服务 | 无法安全证明没有历史直访用户，因此保留并由审计 allowlist 明确隔离；脚本已外置以遵守 CSP，不被主应用 import，也不计入当前主题 |
