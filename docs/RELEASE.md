...
4. 确认 `elytrue-data`、`elytrue-uploads` 两个 Blob Store 和 `ELYTRUE_RATE_LIMIT_KV` 绑定到目标生产项目；普通发布不读取、导出或迁移生产 Blob。
...
完整开发依赖审计用于报告 EdgeOne CLI 等工具依赖风险，不使用 `npm audit fix --force`。
...
- 安全头：HTML CSP、HSTS、API `no-store`、`/assets/*` immutable 和 `/res/*` 重新验证符合 `edgeone.json`。

## 发布后观察

观察 Cloud Functions 错误率、30 秒超时、Edge KV 限流、图片上传失败、`usage-repair-needed`、留言点赞 repair marker、账号 mutation claim 和静态资源 404。日志不得包含密码、恢复密钥、完整邮箱、Cookie 或 API Token。
