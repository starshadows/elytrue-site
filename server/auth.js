...
import { claimUserUid, ensureUserUidSchema } from './user-uids.js'
import {
    normalizeEmail,
    normalizeUsername,
    validateEmail,
...
export async function registerUser(data, env, { name, email, password }) {
    ...
    await ensureUserUidSchema(data, hasExistingUsers)
    ...
    try {
        ...
        emailReserved = true
        uidReservation = await claimUserUid(data, userId)

        const now = Date.now()
        const recoveryKey = generateRecoveryKey()
        const user = {
            id: userId,
            uid: uidReservation.uid,
            name: normalizedName,
        ...
        try {
            ...
            const persisted = await getJSON(data, blobKeys.user(userId)).catch(() => null)
            if (persisted?.id !== userId || persisted?.uid !== uidReservation.uid) throw error
            Object.assign(user, persisted)
            userPersisted = true
        }
        await uidReservation.commit()
        ...
        const conflictMessage = !nameReserved
            ...
            : !emailReserved
                ...
                : '账号创建失败'
        if (!userPersisted) {
            await uidReservation?.rollback().catch(() => {})
