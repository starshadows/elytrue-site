...
export function createCommentRepository(data) {
    return Object.freeze({
        ...
        getPreviousLatest: publicRead => (publicRead ? getJSONPublic : getJSON)(
            data,
            blobKeys.commentsPreviousLatestView,
        ),
        setPreviousLatest: value =>
            data.setJSON(blobKeys.commentsPreviousLatestView, value),
        getDailyCount: (date, publicRead = false) => (publicRead ? getJSONPublic : getJSON)(
            data,
            blobKeys.commentsDailyCount(date),
        ),
        setDailyCount: (date, value) =>
            data.setJSON(blobKeys.commentsDailyCount(date), value),
        getLatestRevision: () => getJSON(data, blobKeys.commentsLatestRevision),
        setLatestRevision: value =>
            data.setJSON(blobKeys.commentsLatestRevision, value),
        setPublicView: (commentId, value) =>
            data.setJSON(blobKeys.commentPublicView(commentId), value),
        getPublicView: commentId =>
            getJSON(data, blobKeys.commentPublicView(commentId)),
