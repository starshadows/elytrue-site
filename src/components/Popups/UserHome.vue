...
const initialProfile = props.profile
  ...
  : props.optimisticProfile
    ? {
        ...
        uid: props.optimisticProfile.uid,
...
const profileReady = ref(Boolean(props.profile || props.optimisticProfile))
...
const displayUid = computed(() => user.value.uid ?? user.value.id)
