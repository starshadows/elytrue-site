const fpsCallbacks: Array<(fps: number) => void> = []
...
export class GallerySwipeController {
  ...
  enable() {
    this.element.addEventListener('touchstart', this._touchStartHandler, {
      passive: false,
    })
  ...
  private _touchMoveHandler = (e: TouchEvent) => {
    ...
    if (
      deltaX == undefined &&
      deltaY == undefined && // first touch
      this._touchStartX != undefined &&
      this._touchStartY != undefined
    ) {
      const XYdiff =
        Math.abs(touch.clientX - this._touchStartX) -
        Math.abs(touch.clientY - this._touchStartY)
      if (
  ...
  private _touchEndHandler = () => {
    ...
    if (
      this._currentTouchSwipeDirectionCorrect &&
      this.options.getStopPosition
    ) {
      ...
      if (
        ...
        let touchSpeed
        if (
          this.options.getItemSize &&
          Math.abs(totalTouchDelta) / this.options.getItemSize() < 0.33
        ) {
        ...
        if (this.options.getItemSize) {
          ...
          itemDelta /= this.isDirectionHorizontal
            ? window.innerWidth
            : window.innerHeight
        }
        // console.log(itemDelta)

        if (itemDelta > this.options.maxItemDelta)
          itemDelta = this.options.maxItemDelta
