...
function viewArtwork(): void {
  const source = image.value
  if (source?.dataset.artwork === 'true' && source.src) {
    ImgViewer.view(source.src)
  }
}
...
<template>
  <div id="musicPlayer">
    <audio id="musicAudio" ref="audio" preload="none"></audio>
    <img id="musicImg" ref="image" @click="viewArtwork" />
    <div>
      <div class="currentSong">No Music Playing</div>
      <ProgressSlider
        id="nowPlayingProgress"
        ref="progress"
        :on-change="(value) => musicController.seek(value)"
      />
    </div>
