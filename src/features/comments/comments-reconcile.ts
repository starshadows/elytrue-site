...
export interface CommentReconcileOptions {
  preserveMissing?: (item: CommentRecord) => boolean
}
...
export function reconcileComments(
  current: CommentRecord[],
  incoming: CommentRecord[],
  options: CommentReconcileOptions = {},
  ...
  for (const item of current) {
    ...
    if (options.preserveMissing?.(item)) items.push(item)
