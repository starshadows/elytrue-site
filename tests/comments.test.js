...
describe('comment read-model functionality', () => {
    it('creates complete canonical, number, public, user and latest records', async () => {
        ...
        assert.deepEqual(publicView, created)
        assert.deepEqual(userView, created)
...
describe('comment read operation budgets', () => {
    ...
    it('falls back from a missing or corrupt latest view with one list and at most count view gets', async () => {
        for (const corrupt of [false, true]) {
            const data = new InstrumentedStore()
            await seed(data, 15)
            if (corrupt) await data.setJSON(blobKeys.commentsLatestView, { version: 1, items: 'bad' })
            else await data.delete(blobKeys.commentsLatestView)
            data.enabled = true
            const page = await listComments(data, new URLSearchParams({ count: '10' }), null, {
                publicRead: true,
            })
            ...
            assert.equal(data.getKeys.filter(key => key.startsWith('views/comments/public/')).length, 10)
            assert.ok(data.maxActiveReads > 1)
            assert.ok(data.maxActiveReads <= 8)
        }
    })

    it('rejects a structurally valid snapshot from a previous Shanghai date', async () => {
        const data = new InstrumentedStore()
        await seed(data, 3)
        const latest = await data.get(blobKeys.commentsLatestView, { type: 'json' })
        ...
        assert.equal(data.listOptions.length, 1)
        assert.equal(data.getKeys.filter(key => key.startsWith('views/comments/public/')).length, 3)
    })
...
describe('comment read-model repair behavior', () => {
    ...
    it('returns readable Server-Timing categories', async () => {
        ...
        for (const category of [
            'routing', 'index', 'readView', 'latestView', 'commentBodies',
            'likes', 'todayCount', 'serialization', 'total',
