...
test('middleware applies document-only CSP while keeping API and binary responses distinct', async () => {
    ...
    const binaryContext = createContext(
        '/api/data/images/avatars/default-avatar.png',
        new MemoryKV(),
    )
...
test('middleware fast-paths static GETs after canonical-host handling', async () => {
    const expected = new Response(new Uint8Array([1]), {
        headers: { 'content-type': 'image/webp' },
    })
    const context = createContext(
        '/assets/elytrue-20260724/bg/landscape1.webp',
        new MemoryKV(),
        '',
        { method: 'GET' },
    )
    context.next = () => expected
    assert.equal(await middleware(context), expected)

    context.request = new Request(
        'https://www.elytrue.com/assets/elytrue-20260724/bg/landscape1.webp',
    )
    context.redirect = (url, status) => new Response(url, { status })
    const redirected = await middleware(context)
    assert.equal(redirected.status, 301)
    assert.equal(
        await redirected.text(),
        'https://elytrue.com/assets/elytrue-20260724/bg/landscape1.webp',
    )
})
