...
test('hint renders immediately and user comments do not wait for auth', async ({
  ...
  await expect(userHome.locator('.userinfo')).toContainText('UID: 101')
...
test('legacy profiles without a numeric uid keep their existing identifier visible', async ({
  page,
}) => {
  const legacyProfile = profile({ id: 'legacy-user-id', name: '旧账号' })
  delete legacyProfile.uid
  await installApiRoutes(page, { authProfile: legacyProfile })

  await openUser(page)
  await expect(page.locator('#popups .userHome .userinfo')).toContainText(
    'UID: legacy-user-id',
  )
})
