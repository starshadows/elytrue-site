...
function seedHomeComments(
  ...
  return page.addInitScript(
    ...
    { items, savedAt, localConfirmations },
  )
}
...
async function mockStartup(
  ...
  await page.route('**/api/comments/public*', (route) =>
    fulfillComments(route, {
      delay,
      items,
      status: commentsError ? 503 : status,
      todayCount,
    }),
  )
  await page.route('**/api/user/me', (route) =>
    fulfillMe(route, { delay, profile, status }),
  )
  await page.route('**/api/comments/count', async (route) => {
    await route.fulfill({
      ...
      body: JSON.stringify({
        code: 1,
        message: 'OK',
        data: typeof todayCount === 'number' ? todayCount : 0,
      }),
    })
  })
}
...
test('首次加载:公共留言、认证和计数独立单飞且不误触分页', async ({ page }) => {
  ...
  await page.route(/\/api\/comments(?:\/[^?]+)?(?:\?.*)?$/u, async (route) => {
    ...
    if (url.pathname === '/api/comments/public-fast')
      fastPublicListRequests += 1
    ...
    if (
      url.pathname === '/api/comments/public-fast' ||
      url.pathname === '/api/comments/public'
    ) {
...
test('页面首次入场独立于认证和留言请求,且只播放一次', async ({ page }) => {
  await mockStartup(page, { delay: 1500 })
  ...
  await page.locator('#themeList [data-theme="auto"]').click()
  await page.evaluate(() => {
...
test('30 秒会话快照在 5 秒公共请求期间赶上入场动画并保持 DOM 身份', async ({
  page,
}) => {
  const cached = commentPayload(41)
  let fastPublicRequests = 0
  let publicRequests = 0
  let countRequests = 0
  page.on('request', (request) => {
    const pathname = new URL(request.url()).pathname
    if (pathname === '/api/comments/public-fast') fastPublicRequests += 1
    if (pathname === '/api/comments/public') publicRequests += 1
    if (pathname === '/api/comments/count') countRequests += 1
  })
  await seedHomeComments(page, [cached], Date.now() - 30_000)
  await mockStartup(page, { delay: 5000, items: [cached], todayCount: 1 })
  ...
  const timing = await page.evaluate(() => {
    ...
    return {
      cache: at('comments-cache-hydrated'),
      dom: at('initial-comment-dom-ready'),
      animation: at('first-comment-animation-start'),
    }
  })
  expect(timing.cache).toBeGreaterThanOrEqual(0)
  expect(timing.animation).toBeGreaterThanOrEqual(0)
  expect(timing.dom).toBeGreaterThanOrEqual(timing.cache)
  expect(timing.dom - timing.animation).toBeLessThanOrEqual(50)
  ...
  expect(fastPublicRequests).toBe(1)
  expect(publicRequests).toBe(0)
  expect(countRequests).toBe(0)
})

test('public snapshot 实际包含 ID 后 ACK journal,下一版本恢复正常删除', async ({
  page,
}) => {
  ...
  await seedHomeComments(page, [local, stable], createdAt - 1_000, [
    {
      id: local.id,
  ...
  await mockStartup(page, { items: [stable], todayCount: 2 })
  let publicCalls = 0
  let publicResponses = 0
  await page.route('**/api/comments/public*', async (route) => {
    publicCalls += 1
    await fulfillComments(route, {
      delay: 1_000,
      items: publicCalls === 1 ? [local, stable] : [stable],
      todayCount: publicCalls === 1 ? 2 : 1,
      snapshotGeneratedAt: createdAt + 10_000,
      snapshotRevision: publicCalls === 1 ? 4 : 5,
    })
    publicResponses += 1
  })
  ...
  const localCard = page
    ...
    .filter({ hasText: '性能留言 142' })
  await expect(localCard).toBeVisible({ timeout: 500 })
  const localHandle = await localCard.elementHandle()
  await expect.poll(() => publicResponses).toBe(1)
  await expect(localCard).toBeVisible()
  expect(
    await localCard.evaluate(
  ...
  await expect(localCard).toHaveCount(0, { timeout: 2000 })
  await expect(
    page.locator('#comments .commentItem').filter({ hasText: '性能留言 141' }),
  ).toHaveCount(1)
...
test('公共留言后台校准只为新增 ID 入场并移除已删除留言', async ({ page }) => {
  ...
  await mockStartup(page, { items: [cached] })
  await page.route('**/api/comments/public*', (route) => {
    publicCalls += 1
...
test('公共留言缓存损坏或过期时删除缓存并直接等待网络加载', async ({ page }) => {
  ...
  await mockStartup(page, { delay: 300, items: [commentPayload(44)] })
  await page.goto('/', { waitUntil: 'domcontentloaded' })
  await expect(page.locator('#comments .commentSkeleton')).toHaveCount(0)
  await expect(page.locator('#comments .commentItem')).toHaveCount(1, {
    timeout: 2000,
...
test('刷新仅追加新留言且空结果只播放刷新动画', async ({ page }) => {
  ...
  let afterRequests = 0
  const afterQueries = []
  await mockStartup(page, { items: [current] })
...
test('公共留言校准失败时保留缓存并提供非阻塞重试', async ({ page }) => {
  ...
  let calls = 0
  await seedHomeComments(page, [cached])
  await mockStartup(page, { commentsError: true, todayCount: 1 })
  await page.route('**/api/comments/public*', (route) => {
    calls += 1
    return fulfillComments(route, {
...
test('置顶与已有留言只随父级面板整体上升', async ({ page }) => {
  ...
  await seedHomeComments(page, [cached])
  await mockStartup(page, { delay: 1500, items: [cached] })
  await page.goto('/', { waitUntil: 'domcontentloaded' })
  await expect(page.locator('#comments .commentItem').first()).toBeVisible()
...
test('1500ms 公共留言请求期间显示置顶卡且不显示留言占位卡', async ({
  page,
}) => {
  await mockStartup(page, { delay: 1500 })
  await page.goto('/', { waitUntil: 'domcontentloaded' })
...
test('认证未决时重复点击只保留一个弹窗和一个 user/me 请求', async ({
  ...
  let meRequests = 0
  await mockStartup(page, { delay: 800, items: [] })
  await page.route('**/api/user/me', async (route) => {
    meRequests += 1
...
test('缓存用户名在 100ms 内显示,服务端确认未登录后才清除', async ({ page }) => {
  ...
  await mockStartup(page, { delay: 500 })
  await page.goto('/', { waitUntil: 'domcontentloaded' })
...
test('认证请求失败会保留缓存用户名', async ({ page }) => {
  ...
  await mockStartup(page, { status: 503 })
  await page.route('**/api/user/me', (route) =>
...
test('今日统计延迟 3000ms 不阻塞留言卡片', async ({ page }) => {
  await mockStartup(page, { todayCount: null })
  await page.route('**/api/comments/count', async (route) => {
