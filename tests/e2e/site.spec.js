...
async function sendMessageAndWaitNewCard(page, expectedCount) {
    ...
    await expect(page.locator('#comments .commentItem')).toHaveCount(expectedCount)
}
...
test('重复用户名注册被拒绝并提示', async ({ page }) => {
    ...
    await fillRegisterForm(page, user1Name, `other_${Date.now()}@example.com`, PASSWORD)
...
test('登录状态恢复：注册后刷新页面仍保持登录', async ({ page }) => {
    ...
    await registerViaPopup(page, name, `reload_${Date.now()}@example.com`, PASSWORD)
...
test('举报：他人留言可举报，自己的留言无举报按钮', async ({ page }) => {
    ...
    await registerViaPopup(page, reportUserName, `report_${Date.now()}@example.com`, PASSWORD)
...
test('编号跳转：输入编号回车后显示对应留言', async ({ page }) => {
    ...
    await expect(page.locator('#comments .commentItem .id').first()).toHaveText('#1')
    await expect(page.locator('#comments .commentItem .comment').first()).toContainText(user1Message)
})

test('个人主页分页：滚动到底加载更多，显示共 N 条留言', async ({ page, request }) => {
