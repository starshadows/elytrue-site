...
test('认证初始化：首次加载只请求一次 user/me，匿名编辑器不等待认证', async ({
  page,
}) => {
...
test('背景文字默认隐藏并由显示设置持久控制', async ({ page }) => {
  await page.goto('/')
  await page.evaluate(() => localStorage.removeItem('hideBackgroundCaptions'))
  await page.reload()

  await expect(page.locator('#mainCaptions')).toBeHidden()
  await liftPanel(page)
  await page.locator('#menu').hover()
  await page.locator('#menu').getByText('显示设置').click()
  const checkbox = page.locator('#hideBackgroundCaptions')
  await expect(checkbox).toBeChecked()
  await checkbox.uncheck()
  await expect(page.locator('#mainCaptions')).toBeVisible()
  await expect(page.locator('.defaultCaption.visible')).toBeVisible()
...
test('页面向下滚轮展开,留言横向滚动不拦截置顶正文且箭头只显示可前进方向', async ({
  ...
  await expect
    .poll(() => comments.getAttribute('data-seek-animating'))
    .not.toBe('true')
  await expect
    .poll(() =>
      comments.evaluate((element) => {
        const bounds = element.getBoundingClientRect()
        const padding = Number.parseFloat(getComputedStyle(element).paddingLeft)
        const contentLeft = bounds.left + element.clientLeft + padding
        return Math.min(
          ...Array.from(element.children)
            ...
            .map((child) =>
              Math.abs(child.getBoundingClientRect().left - contentLeft),
            ),
        )
      }),
    )
  ...
  await expect
    .poll(
      async () => {
        ...
        return page.getByRole('button', { name: '下一页留言' }).count()
      },
      { timeout: 10_000 },
    )
    .toBe(0)
