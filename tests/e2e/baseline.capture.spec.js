...
async function waitForImages(locator) {
  await locator.evaluateAll(async (images) => {
    await Promise.all(
...
async function liftPanel(page) {
  await page.waitForFunction(
    () =>
      !document.querySelector('#lowerPanel')?.classList.contains('animating'),
  )
...
test.skip(
  ...
  'Only runs when explicitly capturing the pre-refactor baseline',
)

test('capture pre-refactor desktop and mobile states', async ({
  page,
  context,
}) => {
  ...
  await waitForImages(page.locator('#themeList img'))
  ...
  const firstBackgroundPreview = page.locator('#getImgPopup img').first()
  await firstBackgroundPreview.scrollIntoViewIfNeeded()
  await waitForImages(firstBackgroundPreview)
  await page.locator('#getImgPopup').evaluate((popup) => {
    popup.scrollTop = 0
  })
