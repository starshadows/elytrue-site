...
describe('EdgeOne account and session API', () => {
    ...
    it('registers immediately with required username, email and password', async () => {
        ...
        assert.equal(payload.data.uid, 1)
        assert.match(payload.data.id, /^[a-f0-9]{8}(?:-[a-f0-9]{4}){3}-[a-f0-9]{12}$/u)
...
describe('EdgeOne comments, uploads and moderation API', () => {
    ...
    it('allows anonymous text posts and replies without creating a user view', async () => {
        const guest = createState('10.0.1.100')
        const first = await call(guest, 'POST', 'comments/post', { comment: '匿名你好' })
        assert.equal(first.response.status, 201)
        assert.equal(first.payload.data.uid, null)
        assert.equal(first.payload.data.sender, '匿名用户')
        assert.equal(first.payload.data.avatar, '')

        const reply = await call(guest, 'POST', 'comments/post', {
            comment: '匿名回复',
            replyid: first.payload.data.displayId,
        })
        assert.equal(reply.response.status, 201)
        assert.equal(reply.payload.data.replyPreview.comment, '匿名你好')
        assert.equal(
            (await guest.stores.data.list({ prefix: 'views/comments/users/' })).blobs.length,
            0,
        )

        const image = await call(guest, 'POST', 'comments/post', {
            comment: '伪造图片',
            imageKeys: ['not-owned'],
        })
        assert.equal(image.response.status, 401)
        const forged = await call(guest, 'POST', 'comments/post', {
            comment: '伪造身份',
            uid: 'admin',
            sender: '管理员',
            role: 'admin',
...
describe('concurrent uniqueness checks', () => {
    it('assigns exactly one admin during concurrent first registrations', async () => {
        ...
        assert.deepEqual(
            results.map(result => result.payload.data.uid).sort((left, right) => left - right),
            [1, 2],
        )
        assert.equal(new Set(results.map(result => result.payload.data.id)).size, 2)
        for (const result of results) {
            const seat = await stores.data.get(
                `indexes/users/uid/${result.payload.data.uid}.json`,
                { type: 'json' },
            )
            assert.equal(seat.userId, result.payload.data.id)
    ...
    it('pauses registration in an existing store until UID migration completes', async () => {
        const stores = { data: new MemoryStore(), uploads: new MemoryStore() }
        ...
        assert.match(result.payload.message, /UID 迁移尚未完成/u)
        assert.equal(
            await stores.data.get('system/admin-bootstrap-closed.json', { type: 'json' }),
            null,
        )
        assert.equal(
            await stores.data.get('meta/users-uid-schema.json', { type: 'json' }),
            null,
        )
    })

    it('continues numbering after a completed legacy-user migration', async () => {
        const stores = { data: new MemoryStore(), uploads: new MemoryStore() }
        const legacyId = '11111111-1111-4111-8111-111111111111'
        await stores.data.setJSON(`users/${legacyId}.json`, {
            id: legacyId,
            name: '迁移前用户',
            role: 'user',
            createdAt: 1_700_000_000_000,
        })
        const migrated = await migrateUserUids(stores.data, {
            fix: true,
            confirmProductionMigration: true,
        })
        assert.equal(migrated.aborted, false)

        const state = createState('10.0.2.15')
        state.stores = stores
        const result = await call(state, 'POST', 'user/register', {
            name: '迁移后用户',
            email: 'after-uid-migration@example.com',
            password: 'secure-password-after-migration',
        })
        assert.equal(result.response.status, 201)
        assert.equal(result.payload.data.uid, 2)
        assert.equal(result.payload.data.role, 'user')
    })

    it('rolls back an unpersisted UID reservation and reuses the number', async () => {
        let failUserWrite = true
        const stores = {
            data: new FlakyStore({
                setJSON: key => failUserWrite && key.startsWith('users/'),
            }),
            uploads: new MemoryStore(),
        }
        const failed = createState('10.0.2.13')
        failed.stores = stores
        const failure = await call(failed, 'POST', 'user/register', {
            name: '回滚占号',
            email: 'uid-rollback@example.com',
            password: 'secure-password-rollback',
        })
        assert.equal(failure.response.status, 500)
        assert.equal(
            await stores.data.get('indexes/users/uid/1.json', { type: 'json' }),
            null,
        )

        failUserWrite = false
        const retried = createState('10.0.2.14')
        retried.stores = stores
        const success = await call(retried, 'POST', 'user/register', {
            name: '回滚占号',
            email: 'uid-rollback@example.com',
            password: 'secure-password-rollback',
        })
        assert.equal(success.response.status, 201)
        assert.equal(success.payload.data.uid, 1)
    })
